...
const apiFetch = AuthService.apiFetch;
...
const fetchRelationships = async (filter: RelationshipFilter, skipCache = false): Promise<CareRelationship[]> => {
  ...
  if (!hasPsych && !hasPat) {
      throw new Error('psychologist_user_id o patient_user_id requerido');
  }
  ...
  if (USE_BACKEND) {
      ...
      if (filter.psychologist_user_id) {
        params.append('psychologist_user_id', filter.psychologist_user_id);
...
const ensureLocalRelationship = (psychUserId: string, patientUserId: string): CareRelationship => {
  ...
  const existing = relationships.find(rel => {
    const relPsychId = rel.psychologist_user_id || rel.psychologistId;
    const relPatId = rel.patient_user_id || rel.patientId;
...
const ensureRelationship = async (psychUserId: string, patientUserId: string): Promise<CareRelationship> => {
  ...
  if (USE_BACKEND) {
      try {
          ...
          const res = await apiFetch(`${API_URL}/relationships`, {
              method: 'POST',
...
// Función para limpiar el caché de conexiones (invitaciones y relaciones)
export const clearConnectionsCache = () => {
  console.log('🗑️ [clearConnectionsCache] Limpiando caché de conexiones...');
  relationshipsCache.clear();
  // No guardamos caché de invitaciones, siempre se consulta al servidor con timestamp
};

const removeRelationship = async (psychUserId: string, patientUserId: string): Promise<void> => {
  if (!psychUserId || !patientUserId) {
      throw new Error('psychologist_user_id y patient_user_id son obligatorios para eliminar relación');
  }
  if (USE_BACKEND) {
      ...
      try {
          const res = await apiFetch(`${API_URL}/relationships?${params.toString()}`, { method: 'DELETE' });
          if (!res.ok) {
              const err = await res.json().catch(() => ({}));
...
const relationshipExists = async (psychUserId: string, patientUserId: string): Promise<boolean> => {
  const rels = await fetchRelationships({ psychologist_user_id: psychUserId, patient_user_id: patientUserId });
...
export const saveEntry = async (entry: JournalEntry): Promise<void> => {
  if (USE_BACKEND) {
      try {
          ...
          if (entry.id) {
              const res = await apiFetch(`${API_URL}/entries?id=${entry.id}`, {
                  method: 'PUT', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(entry)
              });
              if (res.ok) return;
          }
          const createRes = await apiFetch(`${API_URL}/entries`, {
...
export const updateEntry = async (updatedEntry: JournalEntry): Promise<void> => {
    if (USE_BACKEND) {
        try {
            const res = await apiFetch(`${API_URL}/entries?id=${updatedEntry.id}`, {
...
export const deleteEntry = async (id: string): Promise<void> => {
    if (USE_BACKEND) {
        try {
            const res = await apiFetch(`${API_URL}/entries?id=${id}`, { method: 'DELETE' });
            if (!res.ok) {
                let details = '';
                try {
                    const data = await res.json();
...
export const getLastDaysEntriesSummary = async (userId: string, days: number): Promise<Partial<JournalEntry>[]> => {
  ...
  return entries.slice(0, days).map(entry => ({
    ...
    advice: entry.advice
    // Excluimos: transcript (puede ser muy largo), file (base64 audio), sentimentScore (eliminado), y otros datos pesados
  }));
};
...
export const migrateLocalToBackend = async (userId: string) => {
    ...
    try {
        ...
        for (const rel of relationshipsToMigrate) {
            try {
                const psychId = rel.psychologist_user_id || rel.psychologistId;
                const patId = rel.patient_user_id || rel.patientId;
                await apiFetch(`${API_URL}/relationships`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ psychologist_user_id: psychId, patient_user_id: patId })
                });
            } catch (err) {
                const psychId = rel.psychologist_user_id || rel.psychologistId;
                const patId = rel.patient_user_id || rel.patientId;
...
export const getGoalsForUser = async (userId: string): Promise<Goal[]> => {
    if (USE_BACKEND) {
        try {
            const res = await apiFetch(`${API_URL}/goals?userId=${userId}`);
...
export const saveUserGoals = async (userId: string, userGoals: Goal[]) => {
    if (USE_BACKEND) {
        try {
            const res = await apiFetch(`${API_URL}/goals-sync`, {
...
export const getSettings = async (userId: string): Promise<UserSettings> => {
    ...
    if (USE_BACKEND) {
        try {
            const res = await apiFetch(`${API_URL}/settings?userId=${userId}`);
            if (res.ok) {
                const data = await res.json();
...
export const sendInvitation = async (
    ...
    const newInv: Invitation = {
        id: crypto.randomUUID(),
        psychologist_user_id: psychUserId || (existingUser && isPsychologistInvite ? existingUser.id : ''),
        ...
        sender_role: role === 'PSYCHOLOGIST' ? 'PATIENT' : 'PSYCHOLOGIST', // role of the SENDER (not recipient)
    ...
    if (USE_BACKEND) {
        try {
            const res = await apiFetch(`${API_URL}/invitations`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(newInv) });
            if (!res.ok) {
                const errorData = await res.json().catch(() => ({}));
                const err: any = new Error(errorData.message || errorData.error || `Error creating invitation (${res.status})`);
...
export const acceptInvitation = async (invitationId: string, userId: string) => {
    ...
    try {
        const psychId = inv.psychologist_user_id || inv.psychologistId;
        if (!psychId) {
    ...
    if (USE_BACKEND) {
        try {
            ...
            const res = await apiFetch(`${API_URL}/invitations?id=${inv.id}`, { 
                method: 'DELETE',
                headers: {
                    'Cache-Control': 'no-cache',
                    'Pragma': 'no-cache'
...
export const rejectInvitation = async (invitationId: string) => {
    if (USE_BACKEND) {
        try {
            const res = await apiFetch(`${API_URL}/invitations?id=${invitationId}`, { 
                method: 'DELETE',
                headers: {
                    'Cache-Control': 'no-cache',
                    'Pragma': 'no-cache'
...
export const endRelationship = async (psychUserId: string, patientUserId: string) => {
    ...
    try {
        const res = await apiFetch(`${API_URL}/relationships/end`, {
            method: 'PATCH',
            headers: {
...
export const getPatientsForPsychologist = async (psychId: string, includeInactive: boolean = false): Promise<PatientSummary[]> => {
    try {
        ...
        // Usar el endpoint directo que ya incluye toda la información necesaria
        const response = await apiFetch(`${API_URL}/psychologist/${psychId}/patients?showInactive=${includeInactive}`);
        if (!response.ok) {
            console.error('[getPatientsForPsychologist] Error en respuesta:', response.status);
            return [];
        }
        
        const patients = await response.json();
        console.log('[getPatientsForPsychologist] Pacientes obtenidos:', patients);
        
        // Cargar el psicólogo para incluirlo en la lista
        ...
        const userIdsToLoad = [psychId, ...patientIds];
        ...
        const processPatient = (patientData: any): PatientSummary => {
            const userEntries = entriesByUser.get(patientData.id) || [];
            ...
            return {
                ...
                averageSentiment: 0, // DEPRECATED: Ya no se calcula
                recentSummary: lastEntry ? lastEntry.summary : 'No hay registros recientes.',
                riskLevel: 'LOW', // DEPRECATED: Ya no se calcula basado en sentimentScore
                isSelf: false,
        ...
        patients.forEach((p: any) => {
            patientsData.push(processPatient(p));
        });

        // Agregar el psicólogo al principio
        const psychEntries = entriesByUser.get(psychId) || [];
        const lastPsychEntry = psychEntries[0];
        patientsData.unshift({
            id: psych.id,
            name: `${psych.name} (Tú)`,
            email: psych.email,
            phone: psych.phone || '',
            avatarUrl: psych.avatarUrl,
            lastUpdate: lastPsychEntry ? lastPsychEntry.date : 'Sin datos',
            averageSentiment: 0,
            recentSummary: lastPsychEntry ? lastPsychEntry.summary : 'No hay registros recientes.',
            riskLevel: 'LOW',
            isSelf: true,
            tags: [],
            active: true,
