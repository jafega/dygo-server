...
const getDefaultApiUrl = () => {
	...
	// Local dev on desktop — use relative /api so Vite proxy forwards to backend:3001
	// regardless of which port Vite grabbed
...
// Base URL without the /api suffix (used for direct endpoint calls like /api/subscription)
export const API_BASE_URL = (() => {
  const u = API_URL;
  return u.endsWith('/api') ? u.slice(0, -4) : u.replace(/\/api$/, '');
})();
