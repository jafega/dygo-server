...
// Days elapsed since registration → email variant
// day 4  → 10 days remaining
// day 7  → 7 days remaining
// day 11 → 3 days remaining
const REMINDER_SCHEDULE = [
  { daysElapsed: 4,  daysLeft: 10 },
  { daysElapsed: 7,  daysLeft: 7  },
  { daysElapsed: 11, daysLeft: 3  },
];
...
export default async function handler(req, res) {
  ...
  if (!process.env.RESEND_API_KEY) {
    console.error('[trial-reminders] RESEND_API_KEY not set');
    return res.status(500).json({ error: 'RESEND_API_KEY not configured' });
  }
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    console.error('[trial-reminders] Supabase env vars not set');
  ...
  // Fetch all psychologists that don't have an active/trialing subscription
  ...
  if (psychError) {
    console.error('[trial-reminders] Error fetching psychologists:', psychError.message);
    return res.status(500).json({ error: psychError.message });
  }

  // Fetch all active subscriptions to exclude already-subscribed users
  ...
  for (const user of psychologists ?? []) {
    ...
    if (!email || isTempEmail(email)) continue;
    if (subscribedIds.has(user.id)) continue; // already subscribed

    // Determine when the user was created
    let createdAt = null;

    // Prefer auth.users.created_at via admin API
    if (user.auth_user_id) {
      try {
        const { data: authUser, error: authErr } = await supabase.auth.admin.getUserById(user.auth_user_id);
        if (!authErr && authUser?.user?.created_at) {
          createdAt = new Date(authUser.user.created_at).getTime();
        }
      } catch (_) { /* ignore */ }
    }

    // Fallback to data.createdAt
    if (!createdAt) {
      const raw = user.data?.createdAt || user.data?.created_at;
      if (raw) createdAt = typeof raw === 'number' ? raw : new Date(raw).getTime();
    }

    if (!createdAt || isNaN(createdAt)) continue;

    const daysElapsedExact = (now - createdAt) / (1000 * 60 * 60 * 24);
    // Round to nearest integer so the cron window (±12 h) still hits the target day
    const daysElapsed = Math.round(daysElapsedExact);

    // Only send if trial is still active (< 14 days elapsed)
    if (daysElapsed >= TRIAL_DAYS) continue;

    const match = REMINDER_SCHEDULE.find(s => s.daysElapsed === daysElapsed);
    if (!match) continue;
    ...
    try {
      await resend.emails.send({
        ...
        subject: buildReminderSubject(match.daysLeft, firstName),
        html: buildTrialReminderEmail({ firstName, daysLeft: match.daysLeft, appUrl: process.env.FRONTEND_URL || 'https://mi.mainds.app' })
      });
      console.log(`[trial-reminders] ✉️  Sent ${match.daysLeft}-day reminder to ${email} (day ${daysElapsed} of trial)`);
      sent++;
    } catch (err) {
      console.error(`[trial-reminders] Error sending to ${email}:`, err?.message || err);
...
function buildReminderSubject(daysLeft, firstName) {
  const name = firstName ? `, ${firstName}` : '';
  if (daysLeft === 10) return `${firstName || 'Hola'}! Llevas 4 días en mainds 🚀 — te quedan ${daysLeft} días de prueba`;
  if (daysLeft === 7)  return `Te quedan 7 días de prueba en mainds${name} — aquí lo más útil`;
  return `⏳ Solo quedan ${daysLeft} días de tu prueba gratuita en mainds`;
}

const LOGO_SVG = `<svg viewBox="0 0 1242 641" xmlns="http://www.w3.org/2000/svg" style="width:72px;height:37px;display:inline-block;vertical-align:middle;margin-bottom:16px" fill="white">
...
function buildTrialReminderEmail({ firstName, daysLeft, appUrl }) {
  ...
  if (daysLeft === 10) {
    bodyContent = `
      <p style="margin:0 0 20px;font-size:15px;color:#475569;line-height:1.7">
        Ya llevas 4 días usando mainds. Esperamos que estés descubriendo todo lo que puede hacer por ti.
        Aquí tienes algunas cosas que vale la pena explorar esta semana:
      </p>
      <div style="background:#f8f7ff;border:1px solid #e0ddf7;border-radius:10px;padding:20px;margin-bottom:28px">
        <p style="margin:0 0 12px;font-size:13px;color:#64748b;font-weight:700;text-transform:uppercase;letter-spacing:0.06em">Para sacarle el máximo partido</p>
        <ul style="margin:0;padding-left:18px;color:#475569;font-size:14px;line-height:2">
          <li>Invita a tus pacientes por email y gestiona el historial digital</li>
          <li>Configura recordatorios automáticos de sesiones</li>
          <li>Emite facturas en un clic desde cada sesión</li>
          <li>Usa el diario de voz para tus notas clínicas</li>
        </ul>
      </div>`;
  } else if (daysLeft === 7) {
    bodyContent = `
      <p style="margin:0 0 20px;font-size:15px;color:#475569;line-height:1.7">
        Ya llevas una semana en mainds — te quedan exactamente <strong>7 días</strong> de prueba gratuita.
        Ahora es el momento perfecto para configurar la facturación y tener todo listo antes de que expire.
      </p>
      <div style="background:#f8f7ff;border:1px solid #e0ddf7;border-radius:10px;padding:20px;margin-bottom:28px">
        <p style="margin:0 0 12px;font-size:13px;color:#64748b;font-weight:700;text-transform:uppercase;letter-spacing:0.06em">Lo que aún puedes configurar esta semana</p>
        <ul style="margin:0;padding-left:18px;color:#475569;font-size:14px;line-height:2">
          <li>Añade tu IBAN y datos fiscales en Perfil → Datos de facturación</li>
          <li>Crea tu primera factura de prueba</li>
          <li>Configura tu horario de disponibilidad en Agenda</li>
        </ul>
      </div>`;
  } else {
    bodyContent = `
      <p style="margin:0 0 20px;font-size:15px;color:#475569;line-height:1.7">
        Tu prueba gratuita termina en <strong>${daysLeft} días</strong>. Para no perder el acceso a tus datos, pacientes y sesiones, activa tu suscripción antes de que expire.
      </p>
  ...
  const ctaLabel = daysLeft <= 3 ? 'Activar mi suscripción ahora' : 'Continuar en mainds';
...
<body style="margin:0;padding:0;background:#f1f5f9;font-family:Arial,sans-serif;color:#333">
  <div style="max-width:600px;margin:32px auto;padding:0 16px">
    ...
    <!-- HEADER -->
    <div style="background:${headerBg};color:white;padding:40px 24px;text-align:center;border-radius:12px 12px 0 0">
      ${LOGO_SVG}
      <h1 style="margin:0 0 8px;font-size:22px;font-weight:700;letter-spacing:-0.5px">
        ${isUrgent ? `⏳ Te quedan ${daysLeft} días de prueba` : `Te quedan ${daysLeft} días de prueba gratuita`}
      </h1>
      <p style="margin:0;font-size:14px;opacity:0.85">mainds · Software para psicólogos</p>
    </div>
    ...
    <div style="background:#ffffff;padding:36px 28px;border-radius:0 0 12px 12px;box-shadow:0 4px 16px rgba(0,0,0,0.06)">
      ...
      <div style="text-align:center;margin-bottom:28px">
        <a href="${appUrl}"
           style="display:inline-block;padding:15px 44px;background:${isUrgent ? 'linear-gradient(135deg,#DC2626,#9333EA)' : 'linear-gradient(135deg,#4F46E5,#7C3AED)'};color:white;text-decoration:none;border-radius:10px;font-weight:700;font-size:16px;letter-spacing:0.2px">
          ${ctaLabel}
        </a>
      </div>
      ...
      <div style="padding:16px 20px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;text-align:center">
        <div style="font-size:13px;color:#475569">¿Tienes preguntas sobre los planes? Escríbenos a
