  ...
  if (showEditor) {
    return (
      <div className="h-full flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-3 gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium bg-slate-100 text-slate-700 hover:bg-slate-200 transition-colors border border-slate-200 disabled:opacity-50"
              title="Importar PDF o Word y convertir con IA a Markdown"
            >
              {isImporting ? <Loader2 size={14} className="animate-spin" /> : <Upload size={14} />}
              <span className="hidden sm:inline">{isImporting ? 'Convirtiendo…' : 'Importar PDF/Word'}</span>
              <span className="sm:hidden">{isImporting ? '…' : 'Importar'}</span>
            </button>
            <button
              onClick={() => setPreviewMode(!previewMode)}
              className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors ${
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 sm:px-4 py-1.5 sm:py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 text-xs sm:text-sm font-medium transition-colors"
            >
        ...
        {!previewMode && (
          <div className="mb-3 p-2.5 sm:p-3 bg-slate-50 rounded-xl border border-slate-200">
            ...
            <div className="flex flex-wrap gap-1.5 sm:gap-2">
              {PATIENT_VARS.map(v => (
                ...
                className="flex items-center gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg text-[11px] sm:text-xs font-semibold border bg-indigo-50 text-indigo-700 border-indigo-200 hover:bg-indigo-100 transition-colors"
        ...
        <div className="flex-1 flex flex-col lg:flex-row gap-4 min-h-0 overflow-hidden">
          ...
          {!previewMode && (
            <div className="flex-1 flex flex-col min-h-0">
              ...
              <textarea
                ref={textareaRef}
                className="flex-1 w-full p-3 sm:p-4 bg-white border border-slate-200 rounded-xl font-mono text-xs sm:text-sm resize-none focus:outline-none focus:ring-2 focus:ring-indigo-300 leading-relaxed"
                value={editorContent}
          ...
          <div className={`flex-1 flex flex-col min-h-0 ${!previewMode ? 'hidden lg:flex' : ''}`}>
              ...
              <div
                className="flex-1 p-4 sm:p-6 bg-white border border-slate-200 rounded-xl overflow-y-auto prose prose-slate max-w-none text-slate-800 leading-relaxed text-sm"
                style={{ minHeight: '280px' }}
        ...
        {/* Markdown cheatsheet */}
        <div className="mt-3 p-2.5 sm:p-3 bg-slate-50 rounded-xl text-[10px] sm:text-xs text-slate-500 flex flex-wrap gap-x-3 sm:gap-x-4 gap-y-1">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex items-center gap-1.5 sm:gap-2 bg-white border border-slate-200 rounded-xl p-1 shadow-sm w-full sm:w-auto overflow-x-auto">
          {[
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <button
            ...
            className={`flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-medium border transition-colors ${
              showArchived
          ...
          >
            <Archive size={14} />
            <span className="hidden sm:inline">{showArchived ? 'Ocultar archivados' : `Ver archivados${archivedCount > 0 ? ` (${archivedCount})` : ''}`}</span>
            <span className="sm:hidden">{showArchived ? 'Ocultar' : `Archivados${archivedCount > 0 ? ` (${archivedCount})` : ''}`}</span>
          </button>
          <button
            onClick={openCreateEditor}
            className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors shadow-sm text-xs sm:text-sm font-medium ml-auto sm:ml-0"
          >
            <Plus size={15} />
      ...
      {deletingId !== null && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center sm:p-4">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl p-4 sm:p-6 max-w-sm w-full">
      ...
      {showSendModal && sendingTemplate && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center sm:p-4">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl p-4 sm:p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
            {sendSuccess ? (
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center sm:p-4">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-2xl max-h-[90vh] sm:max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between p-4 sm:p-5 border-b border-slate-200">
              <h3 className="font-bold text-slate-900 text-sm sm:text-base truncate">{title}</h3>
              <button onClick={() => setShowPreview(false)} className="p-1.5 hover:bg-slate-100 rounded-lg">
                <X size={18} className="text-slate-500" />
              </button>
            </div>
            <div
              className="flex-1 overflow-y-auto p-4 sm:p-6 prose prose-slate max-w-none text-slate-800 leading-relaxed text-sm"
              dangerouslySetInnerHTML={{ __html: renderContentWithVars(template.content) }}
            />
            <div className="p-3 sm:p-4 border-t border-slate-200 flex justify-end gap-2 sm:gap-3">
