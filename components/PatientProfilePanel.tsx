...
const PatientProfilePanel: React.FC<PatientProfilePanelProps> = ({ userId }) => {
  ...
  const loadPsychologists = async () => {
    ...
    try {
      const response = await apiFetch(`${API_URL}/patient/${userId}/psychologists`);
      if (response.ok) {
        const data = await response.json();
      ...
      {psychologists.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200 p-4 sm:p-6 space-y-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-base sm:text-lg font-semibold text-slate-900">Mis Psicólogos</h3>
          </div>
          <div className="space-y-3">
            {psychologists.map((psych) => (
              <div key={psych.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg border border-slate-100">
                <div className="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center flex-shrink-0 overflow-hidden">
                  {psych.avatarUrl ? (
                    <img src={psych.avatarUrl} alt={psych.name} className="w-full h-full object-cover" />
                  ) : (
                    <UserIcon size={20} className="text-indigo-600" />
                  )}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-slate-900 truncate">{psych.name}</p>
                  {psych.email && (
