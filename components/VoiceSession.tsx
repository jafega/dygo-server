  ...
  useEffect(() => {
    ...
    const startSession = async () => {
      ...
      try {
        ...
        // 2. Parallelizar: acceso al micrófono + entradas recientes + relación (más rápido)
        console.log('[VoiceSession] Fetching mic + entries + relationship in parallel...');
        const [streamResult, recentEntries, patientRel] = await Promise.all([
          ...
          user ? getLastDaysEntriesSummary(user.id, 2) : Promise.resolve([]),
          user ? getPatientActiveRelationship(user.id) : Promise.resolve(null)
        ]);
        const stream = streamResult as MediaStream;
        streamRef.current = stream;
        console.log('[VoiceSession] ✅ Microphone + entries + relationship ready in parallel');
        console.log('[VoiceSession] Loaded', recentEntries.length, 'entries | AI instructions:', patientRel?.ai_instructions ? 'YES' : 'none');
    ...
    <div className="fixed inset-0 bg-slate-900 z-50 flex flex-col items-center justify-between py-12 px-6 text-white font-sans">
      ...
      <div className="flex flex-col items-center gap-2 mt-8 animate-in fade-in slide-in-from-top-4 duration-500">
        ...
        <h2 className="text-3xl font-light tracking-tight flex items-center gap-2">
            <span className="font-semibold">mainds</span>
        </h2>
      ...
      <div className="relative flex items-center justify-center">
        ...
        {/* Center Avatar - MAINDS LOGO (White & Floating) */}
        <div className="relative z-10 w-40 h-40 flex items-center justify-center">
            {isAiSpeaking ? (
                ...
                <svg viewBox="0 0 1242 641" xmlns="http://www.w3.org/2000/svg" className="w-32 h-32 text-white">
        ...
        {error ? (
            ...
            <>
              <p className="text-slate-400 text-sm max-w-xs animate-pulse">
                  {isAiSpeaking ? "mainds te está hablando..." : "mainds está esperando..."}
              </p>
