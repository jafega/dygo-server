...
Mantén un tono profesional y objetivo.`;
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-[200] flex items-center justify-center backdrop-blur-sm">
      <div className="bg-white w-full max-w-5xl h-[90vh] flex flex-col shadow-2xl rounded-2xl overflow-hidden">
        {/* Header */}
        ...
        {isLoadingEntry ? (
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              {entryMode === 'transcript' && (
                <div>
                  ...
                  <textarea
                    ...
                    rows={12}
                    className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none text-sm"
                  />
                </div>
              )}
              ...
              {entryMode === 'upload' && (
                <div>
                  ...
                  <button
                    ...
                    className="w-full px-4 py-8 border-2 border-dashed border-slate-300 rounded-xl hover:border-purple-400 hover:bg-purple-50 transition-all flex flex-col items-center gap-3 disabled:opacity-50"
                  >
                    {isTranscribing ? (
                      <>
                        <Loader className="animate-spin text-purple-600" size={32} />
                        <span className="text-sm text-slate-600">Transcribiendo...</span>
                      </>
                    ) : (
                      <>
                        ...
                        <div className="text-center">
                          <span className="block text-sm font-medium text-slate-700">
                            {uploadedFile ? uploadedFile.name : 'Haz clic para subir'}
                          </span>
                  ...
                  {transcript && entryMode === 'upload' && (
                    <div className="mt-4">
                      <label className="block text-sm font-semibold text-slate-700 mb-2">Transcript (editable)</label>
                      <textarea
                        value={transcript}
                        onChange={(e) => setTranscript(e.target.value)}
                        rows={12}
                        className="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 resize-none text-sm"
                      />
                    </div>
                  )}
                </div>
              ...
              {entryMode === 'record' && (
                <div>
                  <div className="flex flex-col items-center gap-4 p-8 border-2 border-slate-200 rounded-xl bg-slate-50">
                    {isTranscribing ? (
                      <>
                      ...
                      <>
                        ...
                        {audioBlob && (
                          <div className="p-3 bg-green-50 border border-green-200 rounded-lg flex items-center gap-2">
                            <Check size={16} className="text-green-600" />
                            <span className="text-sm text-green-700">Audio transcrito</span>
                          </div>
                        )}
                      </>
                    )}
              ...
              {transcript && !aiSummary && (
                <button
                  ...
                  className="w-full px-6 py-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-semibold hover:from-indigo-700 hover:to-purple-700 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {isGenerating ? (
                    <>
              ...
              {aiSummary && (
                <div>
                  ...
                  <textarea
                    ...
                    rows={10}
                    className="w-full px-4 py-3 border-2 border-green-200 bg-green-50/50 rounded-xl focus:ring-2 focus:ring-green-500 resize-none text-sm"
                  />
                </div>
              )}
            ...
            <div className="border-t border-slate-200 px-6 py-4 bg-slate-50 flex justify-end gap-3">
              <button
                onClick={onClose}
                className="px-6 py-2.5 border border-slate-300 text-slate-700 rounded-lg font-medium hover:bg-slate-100 transition-all"
              >
                Cancelar
              </button>
              <button
                ...
                className="px-6 py-2.5 bg-purple-600 text-white rounded-lg font-medium hover:bg-purple-700 transition-all flex items-center gap-2 disabled:opacity-50"
              >
                {isSaving ? (
                  <>
                    <Loader className="animate-spin" size={16} />
                    {existingEntry ? 'Actualizando...' : 'Guardando...'}
                  </>
                ) : (
                  <>
                    <Save size={16} />
                    {existingEntry ? 'Actualizar' : 'Guardar'}
                  </>
                )}
              </button>
            </div>
