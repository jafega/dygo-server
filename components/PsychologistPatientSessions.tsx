...
const PsychologistPatientSessions: React.FC<PsychologistPatientSessionsProps> = ({ patientId, psychologistId }) => {
  ...
  // Date range state - no default filter
  const [dateRange, setDateRange] = useState<{ start: string; end: string }>({ start: '', end: '' });
  // Separate input state so typing doesn't immediately trigger API calls
  const [inputDateRange, setInputDateRange] = useState<{ start: string; end: string }>({ start: '', end: '' });
  ...
  const loadSessions = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const entriesResponse = await apiFetch(`${API_URL}/session-entries?target_user_id=${patientId}`);
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            <input
              ...
              onBlur={(e) => setDateRange({ ...dateRange, start: e.target.value })}
            ...
            <input
              ...
              onBlur={(e) => setDateRange({ ...dateRange, end: e.target.value })}
              className="px-2.5 sm:px-3 py-2 border border-slate-300 rounded-lg text-xs sm:text-sm focus:ring-2 focus:ring-purple-500"
            />
            <button
              onClick={() => {
