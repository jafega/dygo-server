...
import ExportPatientsModal from './ExportPatientsModal';
...
const MaindsLogoMini: React.FC<{ className?: string }> = ({ className = 'w-5 h-5' }) => (
  <svg viewBox="0 0 1242 641" xmlns="http://www.w3.org/2000/svg" className={className} fill="currentColor">
...
const PatientDashboard = forwardRef<PatientDashboardHandle, PatientDashboardProps>(({ onImportClick, canCreate = true, onNeedUpgrade }, ref) => {
  ...
  // Invite to mainds
  const [invitePatient, setInvitePatient] = useState<PatientSummary | null>(null);
  const [inviteEmailInput, setInviteEmailInput] = useState('');
  const [isSendingInvite, setIsSendingInvite] = useState(false);
  const [inviteToast, setInviteToast] = useState<{ type: 'success' | 'error'; text: string } | null>(null);
  ...
  const handleCreatePatient = async () => {
    ...
    try {
      ...
      const response = await apiFetch(`${API_URL}/admin/create-patient`, {
        method: 'POST',
        headers: {
  ...
  const handleSendInvite = async () => {
    if (!invitePatient || !currentUser) return;
    setIsSendingInvite(true);
    try {
      const res = await apiFetch(`${API_URL}/users/${invitePatient.id}/invite-to-mainds`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: inviteEmailInput })
      });
      if (res.ok) {
        setInviteToast({ type: 'success', text: `Invitación enviada a ${inviteEmailInput}` });
        await loadData(false);
      } else {
        const err = await res.json().catch(() => ({ error: 'Error desconocido' }));
        setInviteToast({ type: 'error', text: err.error || 'Error al enviar la invitación' });
      }
    } catch {
      setInviteToast({ type: 'error', text: 'Error de conexión' });
    } finally {
      setIsSendingInvite(false);
      setInvitePatient(null);
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
       ...
       <div className="flex justify-between items-center gap-2">
         ...
         <div className="flex items-center gap-2">
           ...
           <button
             ...
             className="inline-flex items-center gap-2 px-3 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-medium shadow-lg hover:shadow-emerald-500/30 transition-all"
             title="Exportar lista de pacientes"
           >
             <Download size={18} />
                ...
                filteredPatients.map(patient => (
                    <div key={patient.id} onClick={() => setSelectedPatientId(patient.id)} className="border border-slate-200 rounded-lg sm:rounded-xl p-3 sm:p-4 hover:shadow-md transition-shadow bg-white cursor-pointer group">
                        <div className="flex items-start gap-3">
                            ...
                            {/* Columna derecha: Indicadores + botón mainds */}
                            <div className="flex flex-col items-end gap-1.5 shrink-0">
                                {patient.lastUpdate && patient.lastUpdate !== 'Sin datos' && (
                                    <div className="flex items-center gap-1 text-[9px] sm:text-xs text-slate-500 whitespace-nowrap">
                                ...
                                {!patient.isSelf && (() => {
                                    ...
                                    if (hasAccount) {
                                        return (
                                            <div
                                              title="Paciente registrado en mainds"
                                              className="w-8 h-8 rounded-full flex items-center justify-center bg-gradient-to-br from-indigo-500 to-purple-600 shadow-sm shadow-indigo-200 cursor-default"
                                              onClick={(e) => e.stopPropagation()}
                                            >
                                              <MaindsLogoMini className="w-4 h-4 text-white" />
                                            </div>
                                        );
                                    }
                                    return (
                                        ...
                                        >
                                          <MaindsLogoMini className="w-4 h-4 text-slate-400" />
                                        </button>
                                    );
                                })()}
                            </div>
       ...
       {/* Invite to mainds confirmation popup */}
       {invitePatient && (
         <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50" onClick={() => setInvitePatient(null)}>
           <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6" onClick={(e) => e.stopPropagation()}>
             <div className="flex flex-col items-center text-center gap-4">
               <div className="w-14 h-14 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-lg shadow-indigo-200">
                 <MaindsLogoMini className="w-7 h-7 text-white" />
               </div>
               <div>
                 <h3 className="text-lg font-bold text-slate-800 mb-1">Invitar a mainds</h3>
                 <p className="text-sm text-slate-600">
                   Invitar a <span className="font-semibold text-indigo-700">{invitePatient.name}</span> a usar mainds
                 </p>
               </div>
               <div className="w-full text-left">
                 <label className="text-xs font-medium text-slate-500 mb-1.5 block">Email al que enviar la invitación</label>
                 <input
                   type="email"
               ...
               <div className="flex gap-3 w-full">
                 ...
                 <button
                   ...
                   disabled={isSendingInvite || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(inviteEmailInput.trim())}
                   className="flex-1 px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-xl disabled:opacity-50 disabled:cursor-not-allowed transition-all font-medium text-sm flex items-center justify-center gap-2"
                 >
                   {isSendingInvite ? (
                     <>
