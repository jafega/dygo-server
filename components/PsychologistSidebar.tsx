...
const PsychologistSidebar: React.FC<PsychologistSidebarProps> = ({ 
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      if (resp?.url) {
        localStorage.setItem('dygo_stripe_return_panel', 'schedule');
        window.location.href = resp.url;
      }
    } catch (_) { setSubActionLoading(false); }
  };

  const handleManage = async () => {
    setSubActionLoading(true);
    try {
      const resp = await createBillingPortalSession();
      if (resp?.url) {
        localStorage.setItem('dygo_stripe_return_panel', 'schedule');
        window.location.href = resp.url;
      }
  ...
  const renderSubscriptionCard = () => {
    ...
    // 0. Usuario master → no mostrar tarjeta
    if (s.is_master) return null;

    // 1. Acceso bloqueado por pago fallido
    if (s.access_blocked) {
      return (
        <div className="mx-1 mb-2 rounded-xl bg-red-50 border border-red-200 p-3 text-xs">
          <p className="font-semibold text-red-700 mb-0.5">⛔ Acceso bloqueado</p>
          <p className="text-red-600 mb-2">{s.blocked_reason || 'No se ha podido procesar el pago de tu suscripción.'}</p>
          <button onClick={handleManage} disabled={subActionLoading} className="w-full py-1.5 rounded-lg bg-red-600 text-white font-medium hover:bg-red-700 disabled:opacity-60 transition-colors">
            {subActionLoading ? 'Cargando…' : 'Resolver pago →'}
          </button>
        </div>
      );
    }

    // 2. Suscripcion activa pero cancelará al final del período
    if (s.is_subscribed && s.cancel_at_period_end) {
      return (
        <div className="mx-1 mb-2 rounded-xl bg-amber-50 border border-amber-200 p-3 text-xs">
          <p className="font-semibold text-amber-700 mb-0.5">⚠️ Suscripción cancelada</p>
          <p className="text-amber-600 mb-2">
            Perderás el acceso el <span className="font-semibold">{formatPeriodEnd(s.current_period_end)}</span>.
          </p>
          <button onClick={handleManage} disabled={subActionLoading} className="w-full py-1.5 rounded-lg bg-amber-500 text-white font-medium hover:bg-amber-600 disabled:opacity-60 transition-colors">
            {subActionLoading ? 'Cargando…' : 'Renovar suscripción →'}
          </button>
        </div>
      );
    }
    ...
    if (s.trial_active) {
      ...
      const urgency = days <= 5;
      const expiryLabel = s.trial_expiry_date ? formatPeriodEnd(s.trial_expiry_date) : null;
      return (
        <div className={`mx-1 mb-2 rounded-xl p-3 text-xs border ${urgency ? 'bg-orange-50 border-orange-200' : 'bg-indigo-50 border-indigo-200'}`}>
          <p className={`font-semibold mb-0.5 ${urgency ? 'text-orange-700' : 'text-indigo-700'}`}>
            {urgency ? '⏳' : '🎯'} Período de prueba
          </p>
          <p className={`mb-1 ${urgency ? 'text-orange-600' : 'text-indigo-600'}`}>
            Te quedan <span className="font-semibold">{days} {days === 1 ? 'día' : 'días'}</span> de prueba gratuita.
          </p>
          {expiryLabel && (
            <p className={`mb-2 ${urgency ? 'text-orange-500' : 'text-indigo-500'}`}>
              Caduca el <span className="font-semibold">{expiryLabel}</span>.
            </p>
          )}
          <button onClick={handleUpgrade} disabled={subActionLoading} className={`w-full py-1.5 rounded-lg font-medium text-white disabled:opacity-60 transition-colors flex items-center justify-center gap-1.5 ${urgency ? 'bg-orange-500 hover:bg-orange-600' : 'bg-indigo-600 hover:bg-indigo-700'}`}>
            <Zap size={12} />
            {subActionLoading ? 'Cargando…' : 'Hazte Premium'}
          </button>
        </div>
      );
    }

    // 5. Trial expirado sin suscripcion (cancelada o nunca suscrito)
    const isCanceled = s.stripe_status === 'canceled';
    const cancelReason = s.blocked_reason || (isCanceled ? 'Plan cancelado al final del período.' : null);
    const expiryLabelExpired = s.trial_expiry_date ? formatPeriodEnd(s.trial_expiry_date) : null;
    return (
      <div className="mx-1 mb-2 rounded-xl bg-slate-100 border border-slate-200 p-3 text-xs">
        <p className="font-semibold text-slate-700 mb-0.5">
          {isCanceled ? '🔒 Suscripción cancelada' : '🔒 Sin suscripción activa'}
        </p>
        {cancelReason && <p className="text-slate-500 mb-1">{cancelReason}</p>}
        {!cancelReason && <p className="text-slate-500 mb-1">Tu período de prueba ha finalizado.</p>}
        {expiryLabelExpired && (
          <p className="text-slate-400 mb-2">Caducó el <span className="font-semibold">{expiryLabelExpired}</span>.</p>
        )}
        <button onClick={handleUpgrade} disabled={subActionLoading} className="w-full py-1.5 rounded-lg bg-indigo-600 text-white font-medium hover:bg-indigo-700 disabled:opacity-60 transition-colors flex items-center justify-center gap-1.5">
          <RefreshCw size={12} />
          {subActionLoading ? 'Cargando…' : isCanceled ? 'Reactivar suscripción →' : 'Suscribirse →'}
        </button>
      </div>
    );
  };
  ...
  return (
    <>
      ...
      {!isOpen && (
        ...
        >
          <DygoLogo className="w-10 h-10 text-white" />
        </button>
      )}
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-gradient-to-br from-indigo-600 to-blue-600 rounded-lg flex items-center justify-center">
                <DygoLogo className="w-9 h-9 text-white" />
              </div>
              <span className="font-dygo text-xl font-bold text-slate-900">mainds <span className="text-purple-600">pro</span></span>
            </div>
